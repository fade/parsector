#+TITLE: Parsector Enhancement Notes - Adding Parsec Features
#+AUTHOR: Brian O'Reilly
#+DATE: 2026-02-01

* Overview

This document tracks the effort to add missing Parsec combinators and features to Parsector.

* Comparison: Parsec vs Parsector

** What Parsector Already Has (Parsec Equivalents)

| Parsector       | Parsec        | Notes                    |
|-----------------+---------------+--------------------------|
| ok              | return/pure   | ✓                        |
| fail            | parserFail    | ✓                        |
| char-if         | satisfy       | ✓                        |
...
| chainr          | chainr        | ✓                        |
| chainr1         | chainr1       | ✓                        |
| digit           | digit         | Parsector supports radix |
| natural         | (in Token)    | ✓                        |
| an-integer      | (in Token)    | ✓                        |
| a-float         | (in Token)    | ✓                        |

** What Parsec Has That Parsector Lacks
...
*** 5. ~skipMany1~
Like ~skipMany~ but requires at least one match. Parsector has ~skip-many~ but not ~skip-many1~.
...
- Parsector has no user state mechanism
...
|--------------------------+-------+-------------|
| parsector/test           |   146 | All passing |
| parsector/test/json      |    22 | All passing |
| parsector/test/literals   |    48 | All passing |
| parsector/test/parsec    |    48 | All passing |
| parsector/test/m3u       |    26 | All passing |
| *Total*                  | *290* | *Passing*   |
...
The ~parsector/test/parsec~ package provides coverage for all new Parsec-compatible combinators:
...
I updated the original parsnip library to bring it in-line with the
facilities defined in the Haskell system called parsec. In doing so, I
broadened the surface area and increased the complexity in the system.
I endeavoured to add test coverage for all of these components, in
line with the standards defined by the base system in Samuel Hunter's
master branch on sourcehut. The upstream author was unresponsive to
contributions, so this fork has been renamed to parsector and
restructured as a package-inferred ASDF system.

Regardless, all of this code is open source under the terms of the
original license (BSD 3 Clause), so you may do with it what you want.
Some of this was to scratch my own itch, which was parsing m3u files
(an ill-specified format by consensus of what exists in the world) and
...
so I extended the library to match parsec (as much as possible) as I
went, such that my excercise solutions would at least match the source
...
are my own. Please report issues at https://github.com/fade/parsector/issues.

-Brian O'Reilly, February 2026.
